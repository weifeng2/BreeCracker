//首次请求http://bj.gsxt.gov.cn/corp-query-search-1.html ，注入了一个cookie__jsl_clearance值，返回了一cookie值__jsluid_h
//再次访问这个URL，并且携带这两个Cookie,注入内容更新cookie值 __jsl_clearance

//下面是获取 __jsl_clearance值的方法
...
//下面来解析其他参数geetest_challenge、geetest_validate、geetest_seccode、token，其他参数都为固定值

//请求 http://bj.gsxt.gov.cn/SearchItemCaptcha?t=1630911493657 获取geetest_challenge的值
//请求 http://api.geetest.com/ajax.php? 获取geetest_validate的值
//geetest_seccode的值拼接即可
 ...
 * 
 */
//上面的json.map(function(item){ return String.fromCharCode(item);}).join("")就是生成token的位置
/**
 * 
 */

//这几个参数和增加了极验的验证码有关，就是简单分析下关键参数，没有需求就不破解了
